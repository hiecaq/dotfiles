...
Todo state keywords. There are basically two types:
+ normal tasks
+ bugfix
#+begin_src emacs-lisp :tangle no :noweb-ref org-mode-init
  (general-setq org-todo-keywords
                '((sequence "TODO(t)" "WAIT(w@/@)" "|" "DONE(d!)")
                  (sequence "ISSUE(i)" "BUG(b@)" "KNOWNCAUSE(k@)" "BLOCKED(l@/@)" "RFC(r@/@)" "|" "MERGED(m!)")
                  ("|" "CANCELED(c@)")))
#+end_src

Now finally, the list of files/directories I want my agenda to track:
#+begin_src emacs-lisp :tangle no :noweb-ref org-mode-init
...
#+begin_src emacs-lisp :tangle no :noweb-ref org-mode-init
  (general-setq org-default-notes-file "~/documents/tracking/notes.org")
#+end_src
